...
@implementation GPActionSheetView
...
- (id)initWithShareHeadOprationWith:(NSArray *)titleArray andImageArry:(NSArray *)imageArr andProTitle:(NSString *)protitle and:(ShowType)type
{
    ...
    if (self) {
        ...
        if (type == GPShowTypeIsShareStyle) {
            [self loadUiConfig];
        }
        else
        {
...
- (void)loadActionSheetUi
{
    ...
    for (NSInteger i = 0; i<_shareBtnTitleArray.count; i++) {
        GPVerButton *button = [GPVerButton buttonWithType:UIButtonTypeCustom];
...
- (void)loadUiConfig
{
    ...
    {
        GPActionButton *button = [GPActionButton buttonWithType:UIButtonTypeCustom];
...
- (UIView *)backGroundView
{
    if (_backGroundView == nil) {
        ...
        if (_showtype == GPShowTypeIsShareStyle) {
